...
public class ScriptRunner {
        ...
        try {
            ...
            while ((line = lineReader.readLine()) != null) {
                ...
                if (lineIsComment(trimmedLine)) {
                    ...
                    // 忽略执行 DELIMITER $$ 或 DELIMITER ; ，,将存储过程作为一个完整的脚本执行
                    if (!trimmedLine.contains("DELIMITER ;") && !trimmedLine.contains("DELIMITER $$")) {
                        script.append(line.substring(0, line.lastIndexOf(delimiter)));
                        script.append(LINE_SEPARATOR);
                        statement.addBatch(script.toString());
                        script.setLength(0);
                    }
            ...
            checkForMissingLineTerminator(script);

            statement.executeBatch();
            commitConnection();
        } catch (Exception e) {
            String message = "Error executing: " + e;
            printlnError(message);
            try {
                rollbackConnection();
            } catch (Exception ex) {
                printlnError("rollback error, " + e.getMessage());
                throw ex;
            }
    ...
    private void print(Object o) {
        if (logWriter != null) {
            String message = String.format("%-15s", o.toString());
